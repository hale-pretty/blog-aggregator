...
const delFeedFollows = `-- name: DelFeedFollows :exec
DELETE FROM feeds_follows WHERE id = $1 AND user_id = $2
`

type DelFeedFollowsParams struct {
	ID     uuid.UUID
	UserID uuid.UUID
}

func (q *Queries) DelFeedFollows(ctx context.Context, arg DelFeedFollowsParams) error {
	_, err := q.db.ExecContext(ctx, delFeedFollows, arg.ID, arg.UserID)
	return err
}

const getFeedFollows = `-- name: GetFeedFollows :many
SELECT id, created_at, updated_at, feed_id, user_id FROM feeds_follows WHERE user_id = $1
`
